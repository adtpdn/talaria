...
sprint: alpha
category: CORE
description: "Register the 'Candy Cannon Survival' mode into the game's mode system and lobby to allow selection and configuration."
modified: "2026-06-03"
---
...
## Solution
Integrate the Gauntlet mode into the existing `GameMode` enum and `LobbyManager` configuration.
...
2. **Lobby Integration:**
    - Add `"Candy Cannon Survival"` to the `available_game_modes` list in `scripts/managers/lobby_manager.gd`.
    - Map the mode to the `"Gauntlet Arena"` area.
3. **Settings Configuration:**
    - Implement lobby settings for `gauntlet_round_duration` (180s), `gauntlet_cannon_interval` (5s), and `gauntlet_volley_size` (5 shots).
...
## Acceptance Criteria
- **UI Verification:** Confirm "Candy Cannon Survival" appears in the Game Mode dropdown and "Gauntlet Arena" is selected.
...
## Migration Checklist
- [ ] Update `scripts/game_mode.gd` enum and string mappings.
- [ ] Update `scripts/managers/lobby_manager.gd` with mode name and area mapping.
- [ ] Implement `gauntlet_*` settings and RPCs in `LobbyManager`.
